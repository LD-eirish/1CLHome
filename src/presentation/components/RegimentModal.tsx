/**
 * Presentation Layer - Regiment Modal Component
...
export function RegimentModal({ regiment, onClose }: Readonly<RegimentModalProps>) {
    ...
    >
      <div className="modal-content regiment-modal">
        <button 
          ...
          onClick={onClose}
          aria-label="Close regiment details"
        >
          ✕
        </button>
        ...
        <div className="modal-header">
          <div style={{display: 'flex', alignItems: 'center', gap: '16px'}}>
            <div style={{width: 80, height: 80, flexShrink: 0}}>
              {regiment.logo ? (
                <img 
                  src={assetPath(regiment.logo)} 
                  alt={`${regiment.abbreviation} logo`} 
                  style={{width: '100%', height: '100%', objectFit: 'contain'}} 
                />
              ) : (
                <img 
                  src={assetPath('1CLLogo.png')} 
                  alt="1CL" 
            ...
            <div>
              <h2 id="regiment-modal-title" style={{margin: 0}}>
                {regiment.name}
              </h2>
              <p style={{margin: '4px 0 0', color: 'var(--muted)'}}>
                {regiment.abbreviation} • {regiment.group === 'central' ? 'Central Group' : 'Associate Group'}
              </p>
            </div>
          </div>
        ...
        <div className="modal-body">
          ...
          <section style={{marginBottom: '20px'}}>
            ...
            <dl style={{display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '8px 16px', margin: 0}}>
              <dt style={{fontWeight: 600, color: 'var(--gold)'}}>Joined:</dt>
              <dd style={{margin: 0}}>
                {new Date(regiment.joinedDate).toLocaleDateString('en-US', { 
                  year: 'numeric', 
                  month: 'long', 
                  day: 'numeric' 
                })}
              ...
              <dt style={{fontWeight: 600, color: 'var(--gold)'}}>Leader:</dt>
              <dd style={{margin: 0}}>{regiment.leader ?? 'TBD'}</dd>
              ...
              {regiment.tags && regiment.tags.length > 0 && regiment.tags[0] !== '' && (
                <>
                  <dt style={{fontWeight: 600, color: 'var(--gold)'}}>Tags:</dt>
                  <dd style={{margin: 0}}>
          ...
          {regiment.discord && (
            <a 
              href={regiment.discord} 
              target="_blank" 
              rel="noopener noreferrer" 
              className="discord-btn"
              aria-label={`Join ${regiment.abbreviation} Discord`}
              style={{marginTop: '16px'}}
            >
              <svg viewBox="0 0 71 55" fill="none" xmlns="http://www.w3.org/2000/svg" focusable="false" aria-hidden="true">
