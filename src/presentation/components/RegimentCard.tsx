...
export function RegimentCard({ regiment, index }: Readonly<RegimentCardProps>) {
  return (
    <article className="regiment-card" style={{ animationDelay: `${index * 0.1}s` }}>
      <div className="regiment-header" style={{display: 'flex', alignItems: 'center', gap: '12px'}}>
        ...
        <div style={{flex: 1}}>
          <h4 style={{margin: 0}}>{regiment.name} ({regiment.abbreviation})</h4>
          <span className="regiment-badge">{regiment.group === 'central' ? 'Central' : 'Associate'}</span>
        </div>
      ...
      <p className="regiment-desc">{regiment.description}</p>
